...
export default function Vision({ session }) {
  ...
  async function addGoal(e) {
    ...
    setNg({ title: '', horizon: '1year', category: 'personal' }); setShowAddGoal(false)
  }
      ...
      {view === 'goals' ? (
        <div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '14px' }}>
            <button onClick={() => setShowAddGoal(true)} style={{ padding: '8px 16px', background: 'var(--gold-300)', border: 'none', borderRadius: '10px', cursor: 'pointer', fontFamily: 'var(--font-sans)', fontSize: '12px', fontWeight: 500, color: 'var(--base-950)' }}>+ Goal</button>
          </div>
          ...
          {(horizonFilter === 'all' ? goals : goals.filter(g => g.horizon === horizonFilter)).length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {(horizonFilter === 'all' ? goals : goals.filter(g => g.horizon === horizonFilter)).map(goal => {
          ...
          {showAddGoal && (
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }} onClick={() => setShowAddGoal(false)}>
              <div style={{ background: 'var(--base-800)', border: '0.5px solid var(--base-600)', borderRadius: '20px', padding: '2rem', width: '100%', maxWidth: '440px' }} onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={addGoal} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <button type="button" onClick={() => setShowAddGoal(false)} style={{ flex: 1, padding: '11px', borderRadius: '12px', border: '0.5px solid var(--base-600)', background: 'transparent', color: 'var(--muted)', fontFamily: 'var(--font-sans)', fontSize: '13px', cursor: 'pointer' }}>Cancel</button>
