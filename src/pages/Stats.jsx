import { useState, useEffect } from 'react'
import { supabase } from '../lib/supabase'
import { format, subDays, startOfWeek, endOfWeek, parseISO } from 'date-fns'

export default function Stats({ session }) {
  const [focusSessions, setFocusSessions] = useState([])
  const [habits, setHabits] = useState([])
  const [logs, setLogs] = useState([])
  const [checkins, setCheckins] = useState([])
  const [tasks, setTasks] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => { fetchAll() }, [])

  async function fetchAll() {
    try {
      const uid = session.user.id
      const start30 = format(subDays(new Date(), 30), 'yyyy-MM-dd')
      const [{ data: fs }, { data: h }, { data: l }, { data: ci }, { data: t }] = await Promise.all([
        supabase.from('focus_sessions').select('*').eq('user_id', uid).gte('date', start30).order('date'),
        supabase.from('habits').select('*').eq('user_id', uid),
        supabase.from('habit_logs').select('*').eq('user_id', uid).gte('completed_on', start30),
        supabase.from('checkins').select('*').eq('user_id', uid).gte('date', start30).order('date'),
        supabase.from('tasks').select('*').eq('user_id', uid).gte('created_at', new Date(Date.now() - 30 * 86400000).toISOString()),
      ])
      setFocusSessions(fs || [])
      setHabits(h || [])
      setLogs(l || [])
      setCheckins(ci || [])
      setTasks(t || [])
    } catch (err) {
      console.error('Stats fetch error:', err)
    } finally {
      setLoading(false)
    }
  }

  const weeklyFocus = Array.from({ length: 7 }, (_, i) => {
    const weekStart = startOfWeek(subDays(new Date(), i * 7))
    const weekEnd = endOfWeek(subDays(new Date(), i * 7))
    const mins = focusSessions.filter(s => {
      const d = parseISO(s.date)
      return d >= weekStart && d <= weekEnd
    }).reduce((sum, s) => sum + (s.duration_minutes || 0), 0)
    return { week: format(weekStart, 'MMM d'), hours: Math.round((mins / 60) * 10) / 10 }
  }).reverse()

  const moodTrend = Array.from({ length: 14 }, (_, i) => {
    const date = format(subDays(new Date(), 13 - i), 'yyyy-MM-dd')
    const ci = checkins.find(c => c.date === date)
    return { date: format(subDays(new Date(), 13 - i), 'd'), mood: ci?.mood || 0, energy: ci?.energy || 0 }
  })

  const emotionCounts = focusSessions.reduce((acc, s) => {
    if (s.emotion) acc[s.emotion] = (acc[s.emotion] || 0) + 1
    return acc
  }, {})
  const emotionData = Object.entries(emotionCounts)
    .map(([e, c]) => ({ emotion: e, count: c }))
    .sort((a, b) => b.count - a.count)

  const habitRate = habits.map(h => {
    const done = logs.filter(l => l.habit_id === h.id).length
    return { name: h.name.slice(0, 22), rate: Math.round((done / 30) * 100) }
  }).sort((a, b) => b.rate - a.rate).slice(0, 6)

  const doneTasks = tasks.filter(t => t.status === 'done').length
  const totalFocusHours = Math.round(focusSessions.reduce((s, x) => s + (x.duration_minutes || 0), 0) / 60 * 10) / 10
  const avgMood = checkins.length ? Math.round(checkins.reduce((s, c) => s + c.mood, 0) / checkins.length * 10) / 10 : null
  const habitStreak = (() => {
    let streak = 0; let d = new Date()
    while (streak < 365) {
      const ds = format(d, 'yyyy-MM-dd')
      const dayDone = habits.length > 0 && habits.every(h => logs.some(l => l.habit_id === h.id && l.completed_on === ds))
      if (!dayDone) break; streak++; d = subDays(d, 1)
    }
    return streak
  })()

  const emColors = { flowing: '#a8c4a0', grinding: '#c9a87c', stuck: '#9eb5d4', anxious: '#d4a5a5', drained: '#b8a8d4', hollow: '#8a7060', disconnected: '#7fc4b0', numb: '#d4b8a0' }
  const maxFocusHours = Math.max(...weeklyFocus.map(w => w.hours), 1)
  const maxEmotion = Math.max(...emotionData.map(e => e.count), 1)

  const card = { background: 'var(--base-800)', border: '0.5px solid var(--base-600)', borderRadius: 'var(--radius-lg)', padding: '1.25rem' }
  const lbl = { fontSize: '9px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--muted)', fontFamily: 'var(--font-sans)', marginBottom: '16px', display: 'block' }

  if (loading) return (
    <div style={{ padding: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
      <p style={{ fontFamily: 'var(--font-serif)', fontSize: '24px', fontStyle: 'italic', color: 'var(--muted)' }}>loading stats...</p>
    </div>
  )

  return (
    <div style={{ padding: '32px', maxWidth: '1100px' }}>
      <div style={{ marginBottom: '28px' }}>
        <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '36px', fontStyle: 'italic', fontWeight: 400, color: 'var(--cream-200)', marginBottom: '4px' }}>Stats</h2>
        <p style={{ color: 'var(--muted)', fontSize: '12px', fontFamily: 'var(--font-sans)' }}>last 30 days ✦</p>
      </div>

      {/* TOP STATS */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '20px' }}>
        {[
          { val: totalFocusHours || 0, label: 'focus hours', sub: 'last 30 days' },
          { val: doneTasks, label: 'tasks crushed', sub: 'last 30 days' },
          { val: avgMood ? `${avgMood}/5` : '—', label: 'avg mood', sub: 'last 30 days' },
          { val: habitStreak, label: 'perfect days', sub: 'all habits done' },
        ].map((s, i) => (
          <div key={i} style={{ ...card, textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-serif)', fontSize: '40px', color: 'var(--gold-300)', fontWeight: 400, lineHeight: 1, marginBottom: '6px' }}>{s.val}</div>
            <div style={{ fontSize: '12px', color: 'var(--cream-200)', fontFamily: 'var(--font-sans)', fontWeight: 500, marginBottom: '2px' }}>{s.label}</div>
            <div style={{ fontSize: '10px', color: 'var(--muted)', fontFamily: 'var(--font-sans)' }}>{s.sub}</div>
          </div>
        ))}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>

        {/* WEEKLY FOCUS — CSS bar chart */}
        <div style={card}>
          <span style={lbl}>Weekly focus hours</span>
          {weeklyFocus.every(w => w.hours === 0) ? (
            <p style={{ color: 'var(--muted)', fontSize: '12px', fontFamily: 'var(--font-sans)', fontStyle: 'italic', textAlign: 'center', padding: '40px 0' }}>no focus sessions yet</p>
          ) : (
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px', height: '140px' }}>
              {weeklyFocus.map((w, i) => (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', height: '100%', justifyContent: 'flex-end' }}>
                  <span style={{ fontSize: '9px', color: 'var(--gold-300)', fontFamily: 'var(--font-sans)' }}>{w.hours > 0 ? w.hours : ''}</span>
                  <div style={{
                    width: '100%', borderRadius: '4px 4px 0 0',
                    background: i === weeklyFocus.length - 1 ? '#c9a87c' : '#3d2a1a',
                    height: `${(w.hours / maxFocusHours) * 100}%`,
                    minHeight: w.hours > 0 ? '4px' : '0',
                    transition: 'height 0.6s ease',
                  }} />
                  <span style={{ fontSize: '8px', color: 'var(--muted)', fontFamily: 'var(--font-sans)', whiteSpace: 'nowrap' }}>{w.week}</span>
                </div>
              ))}
            </div>
          )}
        </div>

        {/* MOOD TREND — dot grid */}
        <div style={card}>
          <span style={lbl}>Mood & energy (14 days)</span>
          {checkins.length === 0 ? (
            <p style={{ color: 'var(--muted)', fontSize: '12px', fontFamily: 'var(--font-sans)', fontStyle: 'italic', textAlign: 'center', padding: '40px 0' }}>no check-ins yet</p>
          ) : (
            <div>
              <div style={{ display: 'flex', gap: '4px', marginBottom: '8px', alignItems: 'flex-end', height: '100px' }}>
                {moodTrend.map((d, i) => (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2px', alignItems: 'center', height: '100%', justifyContent: 'flex-end' }}>
                    {d.mood > 0 && <div style={{ width: '100%', background: '#c9a87c', borderRadius: '2px', height: `${(d.mood / 5) * 100}%`, opacity: 0.8 }} />}
                  </div>
                ))}
              </div>
              <div style={{ display: 'flex', gap: '4px' }}>
                {moodTrend.map((d, i) => (
                  <div key={i} style={{ flex: 1, textAlign: 'center' }}>
                    {d.energy > 0 && <div style={{ width: '100%', height: '3px', background: '#d4a5a5', borderRadius: '99px', opacity: d.energy / 5 }} />}
                    <span style={{ fontSize: '7px', color: 'var(--muted)', fontFamily: 'var(--font-sans)' }}>{d.date}</span>
                  </div>
                ))}
              </div>
              <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '2px', background: '#c9a87c' }} />
                  <span style={{ fontSize: '10px', color: 'var(--muted)', fontFamily: 'var(--font-sans)' }}>mood</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div style={{ width: '8px', height: '3px', borderRadius: '99px', background: '#d4a5a5' }} />
                  <span style={{ fontSize: '10px', color: 'var(--muted)', fontFamily: 'var(--font-sans)' }}>energy</span>
                </div>
              </div>
            </div>
          )}
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>

        {/* HABIT COMPLETION */}
        <div style={card}>
          <span style={lbl}>Habit completion (30 days)</span>
          {habitRate.length === 0 ? (
            <p style={{ color: 'var(--muted)', fontSize: '12px', fontFamily: 'var(--font-sans)', fontStyle: 'italic', textAlign: 'center', padding: '20px 0' }}>no habits yet</p>
          ) : habitRate.map((h, i) => (
            <div key={i} style={{ marginBottom: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                <span style={{ fontSize: '12px', color: 'var(--cream-200)', fontFamily: 'var(--font-sans)' }}>{h.name}</span>
                <span style={{ fontSize: '11px', color: 'var(--gold-300)', fontFamily: 'var(--font-sans)' }}>{h.rate}%</span>
              </div>
              <div style={{ height: '4px', background: 'var(--base-700)', borderRadius: '99px', overflow: 'hidden' }}>
                <div style={{ height: '100%', borderRadius: '99px', background: 'linear-gradient(90deg, #c9a87c, #d4a5a5)', width: `${h.rate}%`, transition: 'width 0.8s ease' }} />
              </div>
            </div>
          ))}
        </div>

        {/* EMOTIONS */}
        <div style={card}>
          <span style={lbl}>Session emotions</span>
          {emotionData.length === 0 ? (
            <p style={{ color: 'var(--muted)', fontSize: '12px', fontFamily: 'var(--font-sans)', fontStyle: 'italic', textAlign: 'center', padding: '20px 0' }}>no sessions logged yet</p>
          ) : emotionData.map((e, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
              <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: emColors[e.emotion] || '#8a7060', flexShrink: 0 }} />
              <span style={{ fontSize: '12px', color: 'var(--cream-200)', fontFamily: 'var(--font-sans)', flex: 1, textTransform: 'capitalize' }}>{e.emotion}</span>
              <div style={{ height: '4px', background: 'var(--base-700)', borderRadius: '99px', overflow: 'hidden', width: '80px' }}>
                <div style={{ height: '100%', borderRadius: '99px', background: emColors[e.emotion] || '#8a7060', width: `${(e.count / maxEmotion) * 100}%` }} />
              </div>
              <span style={{ fontSize: '10px', color: 'var(--muted)', fontFamily: 'var(--font-sans)', minWidth: '16px', textAlign: 'right' }}>{e.count}</span>
            </div>
          ))}
        </div>
      </div>
    </div>
  )
}